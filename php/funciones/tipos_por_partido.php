		...
		if(!empty($html)){
			...
			if($cada_tipo->length > 0){
				foreach($cada_tipo as $row){
				    ...
				    if ($url_tipo != "" AND ($nombre_tipo == "Money Line" /*OR $nombre_tipo == "Point Spread" OR $nombre_tipo == "Total Points"*/ OR $nombre_tipo == "Match Result" /* OR $nombre_tipo == "Asian Handicap" OR $nombre_tipo == "Over/Under"*/ OR $nombre_tipo == "Fight Result (Draw No Bet)") AND $nombre_tipo != "") {
